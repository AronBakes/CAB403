...
int main(int argc, char **argv) {
    ...
    card_shm *shared = (card_shm *)(shm + shm_offset);
    ...
    snprintf(buffer, BUFFER_SIZE, "CARDREADER %d HELLO#", id);
    ...
    while (1) {
        ...
        for (int i = 0; i < 16; i++) {
            if (shared->scanned[i] == '\0') { valid_code = false; }
        }
        ...
        if (valid_code) {
            ...
            // Open TCP
            int cardreader_socket = socket(AF_INET, SOCK_STREAM, 0);
            if (cardreader_socket == -1) {
                perror("Failed to create socket file descriptor");
                exit(1);
            }

            if (connect(cardreader_socket, (struct sockaddr *)&overseer_sockaddr, sizeof(overseer_sockaddr)) == -1) {
                perror("Failed to connect");
                close(cardreader_socket);
                exit(1);
            }

            // Scanned message
            snprintf(buffer, BUFFER_SIZE, "CARDREADER %d SCANNED %s#", id, scanned);
            int bytes_sent = send(cardreader_socket, buffer, strlen(buffer), 0);
            if (bytes_sent == -1) {
            ...
            int bytes_received = recv(cardreader_socket, buffer, BUFFER_SIZE, MSG_DONTWAIT);
            if (bytes_received == -1) {
